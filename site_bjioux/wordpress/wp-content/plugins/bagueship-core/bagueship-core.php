<?php
/**
 * Plugin Name: Eclipse Core
 * Description: Bootstrap WordPress/WooCommerce content, collections, legal pages and helper forms for Eclipse.
 * Version: 1.0.0
 * Author: Codex
 */
...
function bagueship_core_admin_menu(): void {
    add_options_page( 'Eclipse', 'Eclipse', 'manage_options', 'bagueship-core', 'bagueship_core_settings_page' );
    add_submenu_page( 'woocommerce', 'Eclipse exports', 'Eclipse exports', 'manage_woocommerce', 'bagueship-exports', 'bagueship_core_exports_page' );
...
function bagueship_core_seed_legal_pages(): void {
    ...
    $mentions .= '<p>Cette page permet d’identifier l’éditeur du site Eclipse et les principaux responsables du service.</p>';
    ...
    $cgu .= '<p>Les présentes conditions encadrent l’accès au site Eclipse, la navigation, les commandes, les précommandes, les paiements, la livraison, les retours et les garanties applicables aux bijoux vendus en ligne.</p>';
    $cgu .= '<h2>1. Identité du vendeur</h2>' . bagueship_core_legal_identity_html( $options );
    $cgu .= '<h2>2. Produits</h2><p>Eclipse vend des bijoux, principalement des bagues en acier inoxydable. Les caractéristiques essentielles sont indiquées sur chaque fiche produit : modèle, matériau, finition, taille, prix, disponibilité, statut de précommande éventuel et visuel 3D lorsque disponible.</p>';
    ...
    $cgu .= '<h2>5. Paiement</h2><p>Le paiement s’effectue via les moyens proposés au moment de la commande. Le site doit afficher clairement le prestataire de paiement actif avant mise en production. Aucune donnée complète de carte bancaire n’est destinée à être stockée par Eclipse.</p>';
    ...
    $cgu .= '<h2>8. Droit de rétractation</h2><p>Conformément aux règles applicables à la vente à distance, le consommateur dispose en principe d’un délai de 14 jours pour exercer son droit de rétractation à compter de la réception du produit. Eclipse ne prévoit pas d’exclusion commerciale spécifique au droit de retour à ce stade, hors limites impératives prévues par la loi. La demande peut être envoyée à : ' . $withdraw_email . '.</p>';
    ...
    $cgu .= '<h2>14. Responsabilité</h2><p>Eclipse s’efforce d’assurer l’accès au site et l’exactitude des informations publiées. Des interruptions techniques, erreurs ou évolutions de stock peuvent toutefois survenir. Les photographies, rendus 3D et couleurs peuvent varier selon les écrans.</p>';
    $cgu .= '<h2>15. Médiation et litiges</h2><p>En cas de litige, le client contacte d’abord Eclipse afin de rechercher une solution amiable. Si le différend persiste, le consommateur peut recourir au médiateur suivant : ' . $mediator . ' - ' . $mediator_url . '.</p>';
    ...
    $privacy .= '<p>Cette politique explique comment Eclipse collecte et utilise les données personnelles nécessaires au fonctionnement du site, aux commandes et aux demandes client.</p>';
    ...
    $ambassador = '<h2>Devenir ambassadeur / ambassadrice</h2><p>Eclipse recherche des profils capables de porter l’univers de la marque avec sincérité : créateurs, modèles, stylistes, photographes, artistes, ou simplement personnes avec une vraie présence visuelle.</p><p>Présentez-vous brièvement, ajoutez vos réseaux et expliquez pourquoi l’univers Eclipse vous parle.</p>[bagueship_ambassador_form]';
    ...
    bagueship_core_create_page( 'À propos', 'a-propos', '<h2>À propos</h2><p>Eclipse construit un univers de bijoux en acier, sculptural, sombre et accessible, avec des pièces pensées comme des objets visuels forts.</p>' );
...
function bagueship_core_seed_site(): void {
    ...
    $product_slugs = array( 'big', 'flamme', 'thin', 'goth' );
    $collection_slugs = array( 'origin' );
    ...
    $collections = array(
        array(
            'name' => 'Origin',
            'slug' => 'origin',
            'meta' => array( 'bagueship_tagline' => 'Quatre formes. Une même origine.', 'bagueship_price_from' => 'À partir de 320 €', 'bagueship_status' => 'Disponible', 'bagueship_collection_state' => 'available', 'bagueship_order' => 1 ),
        ),
    );
    ...
    $products = array(
        ...
        array(
            ...
            'meta' => array(
                'matiere' => 'Acier inoxydable',
                'finition' => 'Argent poli',
                'largeur' => 'Pendentif gothique',
                'poids_bijou' => 'Taille unique',
                'fabrication' => 'Collection Origin',
                'packaging' => 'Livré en écrin Eclipse.',
            ),
        ),
    ...
    foreach ( $products as $product_args ) {
        $product_id = 'simple' === ( $product_args['product_type'] ?? 'variable' )
            ? bagueship_core_create_simple_product( $product_args )
            : bagueship_core_create_variable_product( $product_args );
        if ( $product_id ) {
            update_post_meta( $product_id, '_eclipse_jewel_type', 'goth' === $product_args['slug'] ? 'boucles' : 'bagues' );
        }
    }
...
function bagueship_core_boot_payment_gateway(): void {
    if ( ! class_exists( 'WC_Payment_Gateway' ) || class_exists( 'WC_Gateway_Eclipse_Test' ) ) {
        return;
    }

    class WC_Gateway_Eclipse_Test extends WC_Payment_Gateway {
        public function __construct() {
            $this->id                 = 'bagueship_test';
            $this->method_title       = 'Paiement de test Eclipse';
        ...
        public function process_payment( $order_id ): array {
            ...
            if ( $order ) {
                ...
                if ( 'failure' === $behavior ) {
                    $order->update_status( 'failed', 'Paiement de test Eclipse : échec simulé.' );
                    return array(
                        'result'   => 'failure',
                ...
                $order->add_order_note( 'Commande payée via le gateway de test Eclipse.' );
            }
            return array(
                'result'   => 'success',
    ...
    add_filter(
        ...
        static function ( array $gateways ): array {
            $gateways[] = 'WC_Gateway_Eclipse_Test';
            return $gateways;
        }
    );
}
...
function bagueship_core_contact_handler(): void {
    ...
    wp_mail( $to, 'Contact Eclipse', "Nom: $name\nEmail: $email\n\n$message" );
...
function bagueship_core_ambassador_form_shortcode(): string {
    ...
    $output .= '<p><label>Pourquoi Eclipse ?<br><textarea name="message" rows="6" required></textarea></label></p>';
...
function bagueship_core_ambassador_handler(): void {
    ...
    $message = sanitize_textarea_field( wp_unslash( $_POST['message'] ?? '' ) );
    $to      = get_option( 'admin_email' );
    wp_mail( $to, 'Candidature ambassadeur Eclipse', "Nom: $name\nEmail: $email\nRéseaux: $links\n\n$message" );
...
function bagueship_core_withdrawal_handler(): void {
    ...
    $to           = get_option( 'admin_email' );
    wp_mail( $to, 'Rétractation Eclipse', "Commande: $order_number\nEmail: $email\n\n$details" );
...
function bagueship_core_personal_data_request_handler(): void {
    ...
    $details      = sanitize_textarea_field( wp_unslash( $_POST['details'] ?? '' ) );
    $to           = get_option( 'admin_email' );
    wp_mail( $to, 'Demande données personnelles Eclipse', "Type: $request_type\nNom: $name\nEmail: $email\n\n$details" );
...
function bagueship_core_head_meta(): void {
    ...
    if ( is_product() ) {
        ...
        $description = $options['seo_shop_desc'] ?: 'Boutique Eclipse: collections, bagues variables, panier WooCommerce et checkout invité.';
...
function bagueship_core_cookie_banner(): void {
    ...
    $options = bagueship_core_get_options();
    $message = $options['cookie_text'] ?: 'Eclipse utilise uniquement les cookies techniques nécessaires au panier, au checkout et à la session. Vous pouvez accepter ou refuser les traceurs non essentiels.';
    ?>
...
function bagueship_core_order_status_preparation( int $order_id ): void {
    ...
    $order->add_order_note( 'Commande passée en préparation Eclipse.' );
    bagueship_core_send_order_status_email(
        $order,
        'Votre commande Eclipse est en préparation',
...
function bagueship_core_order_status_expediee( int $order_id ): void {
    ...
    $order->add_order_note( 'Commande marquée expédiée Eclipse.' );
    ...
    bagueship_core_send_order_status_email( $order, 'Votre commande Eclipse a été expédiée', $message );
}
